...
{
  double D0 = 0.0;
  complex<double>  integrant[N];
  // When imaginary part is small, we treat the integral carefully
  if (abs(imag(z))<0.1) 
    D0 = SemiCircleDOS(real(z));

  for (int i=0; i<N ; i++)
    integrant[i]= (d[i] -D0)/(z-w[i]); 
    //integrant[i]= (d[i] )/(z-w[i]);

  //complex<double> sum = 0.0;
  //complex<double> dw = w[2]-w[1]+0j;
  //for (int i=0; i<N-1; i++)
    //sum += dw*integrant[i];

  //return sum;
  return TrapezIntegral( N , integrant , w ) + D0*(log(z-w[0])-log(z-w[N-1]));
}
...
void KramarsKronig( int N, double w[], double imf[], double ref[])
{
    ...
    for ( int i=1; i< (N-1); i++)
        logo[i] = log((w[N-1]-w[i])/(w[i]-w[0]));

    double deriv[N];
    deriv[0] = (imf[1]-imf[0])/dh;
    deriv[N-1] = (imf[N-1]-imf[N-2])/dh;
    for ( int i=1; i< (N-1) ; i++)
...
//-------------------- initialize DOS and Fermifunction-------//
...
double SemiCircleDOS(double x )
{
  ...
  //if ( (sqr(4*t)-sqr(x)) >0 )
  if (abs(x)< 4*t)
    return sqrt(sqr(4*t)-sqr(x))/(2*pi*sqr(2*t));
  else
    return 0.0;
}
